...
class PdfPagesTests(unittest.TestCase):
    def setUp(self):
        ...
        self.output_path = self.directory / "sorties" / "resultat.pdf"
        writer = PdfWriter()
        for _ in range(9):
    ...
    def test_creates_pdf_without_requested_pages(self):
        result = remove_pages(self.input_path, self.output_path, "2,4-6,9")
    ...
    def test_replaces_existing_output(self):
        ...
        remove_pages(self.input_path, self.output_path, "2")
    ...
    def test_rejects_missing_input(self):
        with self.assertRaisesRegex(ValueError, "n'existe pas"):
            remove_pages(self.directory / "absent.pdf", self.output_path, "1")
    ...
    def test_rejects_removing_every_page(self):
        with self.assertRaisesRegex(ValueError, "toutes les pages"):
            remove_pages(self.input_path, self.output_path, "1-9")
    ...
    def test_main_prints_exact_output_path(self):
        ...
        with patch("builtins.print") as print_mock:
            ...
            self.assertEqual(
                main([str(self.input_path), str(self.output_path), "2"]), 0
            )
        print_mock.assert_called_once_with(f"PDF créé : {self.output_path.resolve()}")

    def test_expands_ashell_paths(self):
        from unittest.mock import patch

        home = self.directory / "home"
        input_path = home / "shortcuts" / "entree.pdf"
        input_path.parent.mkdir(parents=True)
        input_path.write_bytes(self.input_path.read_bytes())

        with patch("pathlib.Path.home", return_value=home):
            result = remove_pages(
                "~/shortcuts/entree.pdf", "~/Downloads/resultat.pdf", "2"
            )

        self.assertEqual(result, (home / "Downloads" / "resultat.pdf").resolve())
